...
ggplot() +
  ...
  ggtitle('C Vs A') +xlab('A') +ylab('C')
```

```{r}
ggplot() +
  geom_point(aes(x = dataset1$B, y = dataset1$C),colour = 'Blue') +
  ggtitle('C Vs B') +xlab('B') +ylab('C')
```
```{r}
...
library(ggplot2)
ggplot() +
  geom_point(aes(x = df_test_set$A + df_test_set$A , y = df_test_set$C),
             colour = 'red') +
  geom_line(aes(x = df_test_set$A + df_test_set$A, y = predict(model2, newdata = df_test_set)),
            colour = 'blue') +
...
library(ggplot2)
ggplot() +
  geom_point(aes(x = df_test_set$A, y = df_test_set$C),
             colour = 'red') +
  geom_line(aes(x = df_test_set$A, y = predict(model7, newdata = df_test_set)),
            colour = 'blue') +
  ggtitle('Polynomial Regression') +
  xlab('A5 + A4 + A3 + A2 + A + B') +
  ylab('C')
```
